...
sut_list = [
    ("ride4SBinitramfs", "10.19.243.222", "root", "password"),
]
run_count = 10        # how many test-runs to conduct for each SUT
...
# HACK in initramfs stats
##    cmd_str = "/home/initramfsResults.sh > /home/$$.log"
##    logname = "$(date +"%Y-%m-%dT%H:%M:%S").log"
    FORMAT = '%Y%m%d%H%M%S'
    path = '/home/log'
    log_path = '%s_%s.log' % (path, datetime.datetime.now().strftime(FORMAT))

    cmd = "/home/initramfsResults.sh > "
    cmd_str = cmd + log_path
    stdin, stdout, stderr = client.exec_command(cmd_str, get_pty=True)
    # Block on completion of exec_command
    exit_status = stdout.channel.recv_exit_status()
