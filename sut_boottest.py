...
def dklm(dmesg: str):
    queries = [
            { 'systemd': ('.* running in system mode',0)},
            { 'udev': ('.*Listening on udev Control Socket',0)},
            { 'start_kmod_load': ('.*Starting Load Kernel Modules',0)},
            { 'finish_kmod_load': ('.*Finished Load Kernel Modules',0)},
            ]
...
def clk_ticks(dmesg: str):
    ...
    # Calculate interval
    clk_tick['primary_entry_time_init_int'] = clk_tick['time_init_ts'] - clk_tick['primary_entry_ts']
    return clk_tick
...
def main():
    ...
    while run_number <= run_count:
        ...
        sa_dict, raw_dict  = phase4(sut_ip, sut_usr, sut_pswd, blame_cnt)
